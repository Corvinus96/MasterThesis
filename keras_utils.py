# Taken from
# https://github.com/Simoncarbo/An-experimental-study-of-layer-level-training-speed-and-its-impact-on-generalization/rotation_rate_utils.py
...
def get_kernel_weights(model):
    ...
    return [layer.get_weights()[0] for layer in get_kernel_layer(model)]

def get_layer_weights(layer):
    return layer.get_weights()[0]

def set_layer_weights(layer, new_weights):
...
def plot_layerwise_angle_deviation_curves(deviations, ax=None):
    ...
    for i in range(distances.shape[-1]):
        layer = i
        ax.plot(np.arange(distances.shape[0] + 1), [0] + list(distances[:, layer]), label=str(layer),
                color=cm(cm_inputs[i]))
...
def compute_layerwise_angle_deviation(current_model, initial_w):
    ...
    return [cosine_distance(
                current_model.get_layer(l_name).get_weights()[0].flatten(),
                w.flatten()
            )
...
class LayerwiseAngleDeviationCurves(Callback):
    """
    Computes and saves distance cosine_distance travelled by weights of each layer during training
    """
